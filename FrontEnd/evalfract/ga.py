...
class Fractal:
    ...
    def mutate(self):
        for i in range(len(self.transformations)):
            for j in range(len(self.transformations[i])):
                ...
                self.transformations[i][j] = self.transformations[i][j] + (
                    0.1 * randn()
                )
    ...
    # normalize weight of  fractal
    def normalize(self):
        weigths = [t[-1] for t in self.transformations]
        min_w = min(weigths)
        max_w = max(weigths)
        delta = max_w + min_w
        for i in range(len(self.transformations)):
            self.transformations[i][-1] /= delta
    ...
    def cross_over(cls, parents, n_genes):
        ...
        return cls(
            t_x_parent + choices(transformations, weights, k=n_genes - len(parents))
        )
...
def evolve(population: List[Fractal]):
    ...
    # we normalize all pop
    for fract in final_pop:
        fract.normalize()
